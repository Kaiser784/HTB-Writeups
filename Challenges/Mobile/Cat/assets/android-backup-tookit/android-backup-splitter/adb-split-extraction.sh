...
command -v tr >/dev/null 2>&1 || { echo "tr is required but is not installed. Aborting." >&2; exit 1; }
command -v cut >/dev/null 2>&1 || { echo "cut is required but is not installed. Aborting." >&2; exit 1; }
...
command -v java >/dev/null 2>&1 || { echo "java is not installed, version 7 or higher required. Aborting." >&2; exit 1; }
...
# Java below version 9 uses expresions like 1.7.0_04 or 1.8.0_162, newer ones are like 9.0.4 or 10.0.1
# See "example-java-versions.txt" for details
#JAVA_VERSION_FULL=$(java -version 2>&1 | grep -i version | cut -d \" -f2 | tr -d '\r')
#JAVA_VERSION_HEAD=$(echo $JAVA_VERSION_FULL | cut -d . -f1)
#if [ "$JAVA_VERSION_HEAD" -lt 2 ] ; then
#	# Java version is less than 9, old format, needs checking
#	JAVA_VER=$(echo $JAVA_VERSION_FULL |  cut -d . -f-2 | cut -d . -f2)
#	echo "JAVA_VER="$JAVA_VER
#	if [ "$JAVA_VER" -lt 7 ] ; then
#		echo "java version 7 or higher is required because of SYNC_FLUSH. Aborting."
#		exit 1
#	fi
#fi
...
if [ -z "$1" ]; then 
	...
	echo "java version 7 or higher is required due to SYNC_FLUSH"
	exit 1
fi
if [ ! -f "$1" ]; then
...
### if [ ! -f abp.jar ]; then
if [ ! -f "${SCRIPT_DIR}"/abp.jar ]; then
	echo "abp.jar doesn't exist on present dir. Aborting."
	exit 1
fi

# Backup file, ABP and needed software exist
	...
	### java -jar abp.jar unpack $1 $TARFILE $2
	java -jar "${SCRIPT_DIR}"/abp.jar unpack "$1" "$TARFILE" "$2"
	if [ $? -ne 0 ]; then
	...
	for i in app-tar/* ; do 
		### java -jar abp.jar pack $i app-ab/`basename $i .tar`.ab $2 > /dev/null
		java -jar "${SCRIPT_DIR}"/abp.jar pack "$i" app-ab/`basename "$i" .tar`.ab "$2" > /dev/null
		if [ $? -ne 0 ]; then
